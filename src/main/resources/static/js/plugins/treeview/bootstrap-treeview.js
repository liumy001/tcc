...
;(function($, window, document, undefined) {
	...
	Tree.prototype = {
		...
		_clickHandler: function(event) {
			...
			// 获取是点击的展开按钮还是复选框
			var classL = target.attr('class').indexOf("glyphicon") != -1 ? "0" : "1";


			if(classL == "1") {
				if (node.text.indexOf(" checked ") != -1) {
					var checked = "nochecked";
					node.text = node.text.replace(" checked ", " nochecked ")
				} else {
					node.text = node.text.replace(" nochecked ", " checked ")
					var checked = "checked";
				}

				if (node.text.indexOf(" nochecked ") == -1 && (node.text.indexOf(" checked ") == -1 )) {
					node.text = node.text.replace("<input", "<input checked ")
					var checked = "checked";
				}



				// 如果是父菜单，则处理下面的节点,只有父节点展开才有效
				if(node.text.indexOf(" parent ")){
					var r = node.text.match(/value=\'([^\"]*)\'/);
					var css = "css"+r[1];
					...
					if(node.nodes) {
						for(var i =0;i<node.nodes.length;i++) {
							if (checked == "checked") {
								...
								node.nodes[i].text = node.nodes[i].text.replace(" nochecked ", " checked ")
							} else {
								node.nodes[i].text = node.nodes[i].text.replace(" checked ", " nochecked ")
							}
							if (node.nodes[i].text.indexOf(" nochecked ") == -1 && (node.nodes[i].text.indexOf(" checked ") == -1 )) {
								node.nodes[i].text = node.nodes[i].text.replace("<input", "<input checked ")
								var checked = "checked";
							}
						}
					}
